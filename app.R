...
out <- fst::read_fst("data220831.fst", as.data.table = T, columns = unlist(varlist))
                 ...
                 tabPanel("Spline",
                          sidebarLayout(
                            ...
                            mainPanel(
                              ...
                              h4("Analysis result"),
                              withLoader(verbatimTextOutput("objcoxadd"), type="html", loader="loader6"),
                              withLoader(plotOutput("plotcoxadd"), type="html", loader="loader6"),
                              h4(htmlOutput("convex_coxadd")),
                              h3("Download options"),
                              wellPanel(
                                uiOutput("downloadControls_coxadd"),
                                downloadButton("downloadButton_coxadd", label = "Download the plot")
                              )
                            )
                          )
...
server <- function(input, output, session) {
  ...
  tbsub <-  eventReactive(input$action_tbsub,{
    ...
    tbsub <-  TableSubgroupMultiCox(form, var_subgroups = vs, var_cov = input$cov_tbsub,
  ...
  coxadd.obj <- eventReactive(input$action_coxadd, {
    validate(
      need(!is.null(input$cov_coxadd), "Please select at least 1 independent variable.")
    )
    data <- data()
    label <- data.label()
    ...
    if (!is.null(input$addcov_coxadd)){
      forms.cox <- as.formula(paste(var.day, "~ s(", input$cov_coxadd, ")",  "+", paste(input$addcov_coxadd, collapse = " + ", sep=""))) 
    }
    
    cc <- substitute(mgcv::gam(.form, data= data, family = "cox.ph", weights = get(var.event)), list(.form= forms.cox))
    res.cox <- eval(cc)
    return(res.cox)
  ...
  output$objcoxadd <- renderPrint({
    
    list(summary(coxadd.obj()))
    
  })
  
  sft <- reactive({
    pobj <- plot(coxadd.obj())
    return(min(pobj[[1]]$fit))
  })
  
  
  output$plotcoxadd <- renderPlot({
    
    if (input$hr_coxadd == F){
      plot(coxadd.obj(), ylab = paste0("Log(", "Hazard", ")"), col = "red", shade = T, shade.col="gray90", shift = -sft())
      abline(h = 0, lty = 3) 
      
    } else{
      #cc <- plot(coxadd.obj())
      #with(cc[[1]], matplot(x[-1], cbind(exp(diff(fit)/diff(x))), 
      #                      pch = 16, lty = c(1), col = "black", type = "l", xlab = xlab, ylab = "Hazard ratio"))
      plot(coxadd.obj(), trans = exp, ylab = paste0("Hazard", " ratio"), col = "red", shade = T, shade.col="gray90", shift = -sft())
      abline(h = 1, lty = 3)
      
    }
  })
  
  output$convex_coxadd <- renderText({
    req(input$action_coxadd)
    if (input$hr_coxadd == F){
      pobj <- plot(coxadd.obj(), ylab = "log(HR)") 
      min.x <- round(pobj[[1]]$x[which.min(pobj[[1]]$fit)], 1)
      return(paste("Log(", "Hazard", ")", " 는 <B>", min.x, "</B> 에서 최소값을 갖는다.", sep = ""))
    } else{
      #cc <- plot(coxadd.obj())
      #pobj <- with(cc[[1]], matplot(x[-1], cbind(exp(diff(fit)/diff(x)), exp(diff(fit - 1.96 *se)/diff(x)), exp(diff(fit + 1.96 *se)/diff(x))), 
      #                      pch = 16, lty = c(1, 2, 2), col = "black", type = "l", xlab = xlab, ylab = "Hazard ratio"))
      pobj <- plot(coxadd.obj(), trans = exp, ylab = "Hazard", col = "red", shade = T, shade.col="gray90")
      #min.x <- round(with(cc[[1]], x[-1][which.min(cbind(exp(diff(fit)/diff(x))))]), 1)
      min.x <- round(pobj[[1]]$x[which.min(pobj[[1]]$fit)], 1)
      return(paste( paste0("Hazard", " ratio"), " 는 <B>", min.x, "</B> 에서 최소값을 갖는다.", sep = ""))
    }
    
  })
  
  
  output$downloadControls_coxadd <- renderUI({
    fluidRow(
      column(4,
             selectizeInput("coxadd_file_ext", "File extension (dpi = 300)", 
                            choices = c("jpg","pdf", "tiff", "svg", "emf"), multiple = F, 
                            selected = "emf"
             )
      ),
      column(4,
             sliderInput("fig_width_coxadd", "Width (in):",
                         min = 5, max = 20, value = 8
             )
      ),
      column(4,
             sliderInput("fig_height_coxadd", "Height (in):",
  ...
  output$downloadButton_coxadd <- downloadHandler(
    filename =  function() {
      paste(input$dep_coxadd, "_", input$cov_coxadd , "_plot.", input$coxadd_file_ext ,sep="")
    },
    # content is a function with argument file. content writes the plot to the device
    content = function(file) {
      withProgress(message = 'Download in progress',
                   detail = 'This may take a while...', value = 0, {
                     ...
                     if (input$coxadd_file_ext == "emf"){
                       devEMF::emf(file, width = input$fig_width_coxadd, height =input$fig_height_coxadd, coordDPI = 300, emfPlus = F)
                       if (input$hr_coxadd == F){
                         plot(coxadd.obj(), ylab = paste0("Log(", "Hazard", ")"), col = "red", shade = T, shade.col="gray90", shift = -sft())
                         abline(h = 0, lty = 3) 
                       ...
                       ggsave(file, plot(coxadd.obj()), dpi = 300, units = "in", width = input$fig_width_coxadd, height =input$fig_height_coxadd)
                     }
