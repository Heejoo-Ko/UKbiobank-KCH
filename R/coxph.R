...
coxModule <- function(input, output, session, data, data_label, data_varStruct = NULL, nfactor.limit = 10, design.survey = NULL, default.unires = T, limit.unires = 20, id.cluster = NULL, ties.coxph = "efron") {
  ...
  output$indep <- renderUI({
    ...
    if (is.null(design.survey)){
      indep.cox <- setdiff(names(data()), c(vlist()$except_vars, varlist$Event, varlist$Time))
      if (!is.null(id.cluster)){
        indep.cox <- setdiff(names(data()), c(vlist()$except_vars, varlist$Event, varlist$Time, id.cluster()))
      }
      ...
      indep.cox <- setdiff(names(data()), c(vlist()$except_vars, varlist$Event, varlist$Time, names(design.survey()$allprob), names(design.survey()$strata), names(design.survey()$cluster)))
      if (default.unires){
  ...
  observeEvent(input$subcheck, {
    output$subvar <- renderUI({
      ...
      var_subgroup <- setdiff(names(data()), c(varlist$Event, varlist$Time, input$indep_cox))
      if (!is.null(id.cluster)){
        var_subgroup <- setdiff(names(data()), c(varlist$Event, varlist$Time, input$indep_cox, id.cluster()))
      } else if (!is.null(design.survey)){
        var_subgroup <- setdiff(names(data()), union(c(names(design.survey()$strata), names(design.survey()$cluster), names(design.survey()$allprob)), c(varlist$Event, varlist$Time, input$indep_cox)))
      }
