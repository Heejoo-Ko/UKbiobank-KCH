...
TableSubgroupCox <- function(formula, var_subgroup = NULL, var_cov = NULL, data, time_eventrate = 3 * 365, decimal.hr = 2, decimal.percent = 1, decimal.pvalue = 3){
  ...
  if (is.null(var_subgroup)){
    if (!is.null(var_cov)){
      formula <- as.formula(paste0(paste0(deparse(formula), collapse = ""), " + ", paste(var_cov, collapse = "+")))
    }
    ...
    if (any(class(data) == "survey.design")){
      ...
      pvs_int <- possible_svycoxph(as.formula(gsub(xlabel, paste0(xlabel, "*", var_subgroup), paste0(deparse(formula), collapse = ""))), design = data) %>% summary %>% coefficients
      ...
      model.int <- survey::svycoxph(as.formula(gsub(xlabel, paste0(xlabel, "*", var_subgroup), paste0(deparse(formula), collapse = ""))), design = data)
      
      if (sum(grepl(":", names(coef(model.int)))) > 1){
        model.int$call$formula <- as.formula(gsub(xlabel, paste0(xlabel, "*", var_subgroup), paste0(deparse(formula), collapse = "")))
      ...
      data[!is.na(data[[var_subgroup]]), ] %>% split(.[[var_subgroup]]) %>% purrr::map(~possible_coxph(formula, data = ., x= T)) -> model
      data[!is.na(data[[var_subgroup]]), ] %>% select(var_subgroup) %>% table %>% names -> label_val
      xlev <- survival::coxph(formula, data = data)$xlevels
      model.int <- possible_coxph(as.formula(gsub(xlabel, paste0(xlabel, "*", var_subgroup), paste0(deparse(formula), collapse = ""))), data = data)  
      if (sum(grepl(":", names(coef(model.int)))) == 1){
    ...
    model %>% purrr::map(possible_confint)  %>% lapply(function(x){x[1, ]}) %>% Reduce(rbind, .) %>% exp %>% round(decimal.hr) -> CI
